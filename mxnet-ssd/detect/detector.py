...
class Detector(object):
	...
	def detect_and_save_result (self, im_list, root_dir, extension=None,
		...
		wf=open('../output/output_t1_old.txt','w+',newline='')
		writer=csv.writer(wf)
		wf2=open('../output/output_t2.txt','w+',newline='')
		writer2=csv.writer(wf2)
		writer.writerow(['guid/image','label']) ## for task1
		writer2.writerow(['guid/image','conf','label','xmin','ymin','xmax','ymax']) ## for task2
		...
		for k, det in enumerate(dets):
			...
			if len(det)==0: 
				lbl='{0:d}'.format(int(np.random.choice(3, 1, p=[0.094475, 0.534227, 0.371298])))
				nullcount+=1
				writer2.writerow([guid+'/'+img,str(0),lbl,str(-1),str(-1),str(-1),str(-1)]) ## for task2
			else:
				...
				lbl='{0:d}'.format(int(det[best_idx][0]))
				xmin='{:.3f}'.format(float(det[best_idx][2]))
				ymin='{:.3f}'.format(float(det[best_idx][3]))
				xmax='{:.3f}'.format(float(det[best_idx][4]))
				ymax='{:.3f}'.format(float(det[best_idx][5]))
				writer2.writerow([guid+'/'+img,conf[best_idx],lbl,xmin,ymin,xmax,ymax]) ## for task2
